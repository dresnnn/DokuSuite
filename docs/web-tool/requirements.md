...
- Freigabeverwaltung: bestehende Shares paginiert listen (`page`/`limit`), optional nach Auftrag filtern (`orderId`), Seitenwechsel über `Prev`/`Next`-Buttons mit Gesamtseitenzahl aus `total` und `limit`, neue Links mit Ablaufdatum (`expires_at`, UTC im RFC3339-Format), Download-Erlaubnis (`download_allowed`), Wasserzeichen-Policy (`watermark_policy`) und optional benutzerdefiniertem Wasserzeichen-Text (`watermark_text`). Der `watermark_text` ist ausschließlich bei `watermark_policy=custom_text` erforderlich. Widerruf über `POST /shares/{id}/revoke`; die generierte URL wird nach Erstellung angezeigt und kann per "Copy"-Button in die Zwischenablage kopiert werden. Nach Eingabe der Auftrags-ID werden `GET /orders/{id}` und `GET /customers/{customer_id}` aufgerufen, um die Wasserzeichen-Voreinstellungen des Kunden automatisch zu übernehmen. Fehlschläge dieser API-Aufrufe werden als Fehler-Toast zurückgemeldet. In der Share-Liste werden Ablaufdatum und Wasserzeichen-Policy in eigenen Spalten dargestellt.
- Export-Workflow: Export-Jobs der aktuellen Sitzung lokal verfolgen, in `localStorage` persistieren und bei Bedarf löschen (manuell in allen Ansichten entfernbar), ZIP-, Excel- und PDF-Exporte der ausgewählten Fotos anstoßen (`POST /exports/zip`, `POST /exports/excel`, `POST /exports/pdf`), Status via Polling aktualisieren (`GET /exports/{id}`) und Download-Link bei abgeschlossenen Jobs (`status=done`). ZIP-Exporte unterstützen optional einen Titel (`title`) sowie die Einbindung von EXIF-Daten (`includeExif`). Erfolg und Fehler werden über Toasts rückgemeldet.
- Export-Jobs sind pro Freigabe isoliert und werden im `localStorage` mit token-basierten Keys getrennt gespeichert.
...
- Bei abgelaufener Sitzung (HTTP 401) löscht das Frontend das Token, zeigt einen "Session expired"-Toast und leitet automatisch auf `/login` weiter.
