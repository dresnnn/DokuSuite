...
- Freigabeverwaltung: bestehende Shares paginiert listen (`page`/`limit`), optional nach Auftrag filtern (`orderId`), Seitenwechsel über `Prev`/`Next`-Buttons mit Gesamtseitenzahl aus `total` und `limit`, neue Links mit Ablaufdatum (`expires_at`), Download-Erlaubnis (`download_allowed`), Wasserzeichen-Policy (`watermark_policy`) und optional benutzerdefiniertem Wasserzeichen-Text (`watermark_text`) bei `watermark_policy=custom_text` erzeugen, Widerruf über `POST /shares/{id}/revoke`; die generierte URL wird nach Erstellung angezeigt und kann per "Copy"-Button in die Zwischenablage kopiert werden. Nach Eingabe der Auftrags-ID werden `GET /orders/{id}` und `GET /customers/{customer_id}` aufgerufen, um die Wasserzeichen-Voreinstellungen des Kunden automatisch zu übernehmen.
